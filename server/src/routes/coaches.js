...
router.post('/', (req, res) => {
  ...
  try {
    const result = db.run(
      'INSERT INTO coaches (nom, prenom, email, telephone) VALUES (?, ?, ?, ?)',
...
router.put('/:id', (req, res) => {
  ...
  try {
    db.run(
      'UPDATE coaches SET nom = ?, prenom = ?, email = ?, telephone = ? WHERE id = ?',
      [nom.trim(), prenom.trim(), email?.trim() || null, telephone?.trim() || null, req.params.id]
    );
